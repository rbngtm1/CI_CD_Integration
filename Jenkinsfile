...
node('node'){
   ...
   stage('test case and report'){
      try {
         ...
         publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'addressbook_main/target/', reportFiles: 'surefire-report.html', reportName: 'htmlreports', reportTitles: ''])
      } catch(err) {
         throw err
      }
   }
