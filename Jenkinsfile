...
node('node'){
   ...
   stage('test case and report'){
      try {
         ...
         publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'site', reportFiles: 'surefire-report.html', reportName: 'HTMLReport', reportTitles: ''])
      } catch(err) {
         throw err
      }
   }
