...
echo 'JAVA_OPTS="-Djava.security.egd=file:/dev/./urandom -Djava.awt.headless=true -Xmx1026m -XX:MaxPermSize=256m -XX:+UseConcMarkSweepGC"'>>/usr/share/tomcat/conf/tomcat.conf
# install the web application manager and the documentation,
sudo yum install tomcat-webapps tomcat-admin-webapps
sudo yum install tomcat-docs-webapp tomcat-javadoc
sudo cat /usr/share/tomcat/conf/tomcat-users.xml | sed 's/<tomcat-users>/<tomcat-users>\n<user username="admin" password="password" roles="manager-gui,admin-gui"/> > /usr/placeholder.xml
...
sudo cp jenkins.war /opt/tomcat9/webapps/
